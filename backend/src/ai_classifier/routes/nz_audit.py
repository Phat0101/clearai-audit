...
- Output CSV results
- Support background processing with polling-based progress
"""
import asyncio
    ...
    for item in sorted(grouped_folder.iterdir()):
        if item.is_dir() and item.name.startswith("job_"):
            ...
            for row in existing_results:
                if row.get("HAWB") == job_id or row.get("DHL Job Nmb", "").startswith(job_id[:8]):
                    hawb = row.get("HAWB")
                    break
...
async def process_nz_audit(
    ...
    """Start NZ audit processing as a background task. Returns immediately.

    Use GET /api/nz-audit/status?folder_name=... to poll progress.
    ...
    if folder_name in _active_audits and not _active_audits[folder_name].done():
        ...
        return {
            "success": True,
            "message": "Audit already in progress. Use GET /api/nz-audit/status to check progress.",
            "is_running": True,
    ...
    async def _run_audit():
        try:
            return await process_grouped_jobs_nz(
    ...
    return {
        "success": True,
        "message": f"Audit started in background for {folder_name}. Poll GET /api/nz-audit/status?folder_name={folder_name} for progress.",
        "is_running": True,
    }
...
async def get_audit_result(folder_name: str = Query(...)):
    """Get the final result after audit completes."""
    ...
    return NZAuditBatchResponse(
        success=True,
        message=f"NZ audit complete. Output: {result.get('run_path', '')}",
        run_id=result.get("run_id"),
        run_path=result.get("run_path"),
        total_jobs=result["total_jobs"],
        successful_jobs=result["successful_jobs"],
        failed_jobs=result["failed_jobs"],
        skipped_jobs=result.get("skipped_jobs", 0),
